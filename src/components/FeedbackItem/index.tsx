...
const Item: React.FC<ItemType> = ({ item, handleFeedbackCliick }) => {
    ...
    return (
        <div onClick={() => handleFeedbackCliick?.(item._id)} className={styles.item}>
            <div className={styles.button_box} onClick={(e) => e.stopPropagation()}><Button disabled={!data && true} active={isUpvoted} onClick={() => handleUpvote(dispatch, item, data)} className={'upvotes'}>{item.upvotes}</Button></div>
